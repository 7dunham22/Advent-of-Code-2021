...
def isLowest(rowIndex, colIndex):
    target = heatMap[rowIndex][colIndex]
    if rowIndex > 0 and heatMap[rowIndex-1][colIndex] <= target:
        return False
    if rowIndex < len(heatMap) - 1 and heatMap[rowIndex+1][colIndex] <= target:
        return False
    if colIndex > 0 and heatMap[rowIndex][colIndex-1] <= target:
        return False
    if colIndex < len(heatMap[rowIndex]) - 1 and heatMap[rowIndex][colIndex+1] <= target:
        return False
    return True

heatMap = data.split('\n')[:-1]
heatMap = [[int(char) for char in row] for row in heatMap]
lowPoints = []
for rowIndex in range(len(heatMap)):
    row = heatMap[rowIndex]
    for colIndex in range(len(row)):
        target = heatMap[rowIndex][colIndex]
        if isLowest(rowIndex, colIndex):
