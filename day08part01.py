...
into the cave mouth, collapsing it. Sensors indicate another exit
to this cave at a much greater depth, so you have no choice but to
press on.
...
by producing output on signal wires a through g, but those wires are connected
...
don't know what the connections actually are). The unique signal patterns correspond to
the ten different ways the submarine tries to render a digit using the current wire/segment
connections. Because 7 is the only digit that uses three segments, dab in the above example
 means that to render a 7, signal lines d, a, and b are on. Because 4 is the only digit that
...
for pattern in data:
    ...
    for output in outputs:
        if len(output) == 2:
            translations[output] = 1
        elif len(output) == 3:
            translations[output] = 7
        elif len(output) == 4:
            translations[output] = 4
        elif len(output) == 7:
            translations[output] = 8
    # print(translations)
    for i in range(len(outputs)):
        output = outputs[i]
        if output in translations.keys():
